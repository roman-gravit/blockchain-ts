...
export class WebsocketController {
  ...
  private readonly onMessageReceived = (event: MessageEvent) => {
    ...
    if ((this.messagesAwaitingReply && this.messagesAwaitingReply.has(message.correlationId))) {
		if(message.correlationId && this.messagesAwaitingReply) {
			const key  = message.correlationId || "";
			const obj = this.messagesAwaitingReply.get(key);
			if(obj) {
				obj.resolve(message);
			}
  ...
  async send(message: Partial<Message>, awaitForReply: boolean = false): Promise<Message> {
    return new Promise<Message>(async (resolve: any, reject) => {
	const key = message.correlationId || "";
      if (awaitForReply) {
        this.messagesAwaitingReply.set(key, { resolve, reject });
      }
      this.websocket.then(
        ws => ws.send(JSON.stringify(message)),
        () => this.messagesAwaitingReply.delete(key)
      );
    });
  }
